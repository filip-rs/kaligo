...
from kaligo.util import tg, time
...
class TelegramBot(KaligoBase):
    ...
    async def init_client(self: "Kaligo") -> None:
        ...
        if bot_token:
            ...
            file = AsyncPath("kaligo/kaligo_helper.session")
    ...
    async def start(self: "Kaligo") -> None:
    ...
    async def idle(self: "Kaligo") -> None:
        if self.__idle__:
    ...
    def update_module_events(self: "Kaligo") -> None:
    ...
    @property
    def events_activated(self: "Kaligo") -> int:
        return len(self._mevent_handlers)

    @property
    def helper_initialized(self: "Kaligo") -> bool:
        return hasattr(self, "client_helper") and isinstance(self.client_helper, Client)

    def redact_message(self: "Kaligo", text: str) -> str:
        redacted = "[REDACTED]"
