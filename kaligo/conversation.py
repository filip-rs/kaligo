...
class Conversation:
    ...
    @classmethod
    async def new(
        cls, bot: "Kaligo", input_chat: Union[str, int], timeout: int, max_messages: int
    ) -> "Conversation":
    ...
    async def _get_message(self, **kwargs: Any) -> Message:
        ...
        filters: Optional[Filter] = kwargs.get("filters")
        ...
        while True:
            ...
            if filters is not None:
                ready: bool = await filters(self.bot.client, result)
                if not ready:
                    continue
